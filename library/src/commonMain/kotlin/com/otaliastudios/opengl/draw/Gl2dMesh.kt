...
public open class Gl2dMesh: Gl2dDrawable() {
    ...
    public fun setPoints(x: List<Float>, y: List<Float>) {
        ...
        if (vertexArray.capacity() < coords) {
            vertexArray.dispose()
            vertexArray = floatBuffer(coords)
        } else {
            vertexArray.clear()
        }
    ...
    private fun computeIndicesFromIndexedSegments(segments: List<IndexedSegmentF>) {
        ...
        vertexIndices?.dispose() // TODO reuse it if possible
        vertexIndices = byteBuffer(indices.size).also { buffer ->
            indices.forEach { buffer.put(it) }
            buffer.clear()
        }
    }
    ...
    override fun draw() {
        vertexIndices?.let {
            ...
            glDrawElements(GL_TRIANGLES, it.limit(), GL_UNSIGNED_BYTE, it)
